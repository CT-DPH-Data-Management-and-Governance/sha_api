...
the problem is each column has 1 or more metrics that go only to that
column. so we could append a prefix, but then columns are going to be 
unique to only the subject table itself - can't stack em in sql etc...
